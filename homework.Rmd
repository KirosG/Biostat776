...
* [Sections 1.2 through 1.5 of Mastering Software Development in R](http://bookdown.org/rdpeng/RProgDA/the-importance-of-tidy-data.html)
...
Note that the functions in the `dplyr` and `tidyr` package expect table-like objects (data frames or tibbles) as their input. You may need to convert data to these objects using the `tibble` package.

### Part 2

Use the `readr` package to read the [SPEC_2014.csv.gz](data/SPEC_2014.csv.gz) data file in to R. This file contains daily levels of fine particulate matter (PM2.5) chemical constituents across the United States. The data are measured at a network of federal, state, and local monitors and assembled by the EPA. 
...
* You will need to combine the data in the [aqs_sites.xlsx](data/aqs_sites.xlsx) file with the data from the previous Part in order to answer these questions.
...
* See the [notes on joining data](notes/pdf/join_data.pdf) for examples of how these functions work.
